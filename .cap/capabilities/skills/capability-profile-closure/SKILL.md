...
2. 检查声明闭包：
   ...
   - 每个引用能力都存在于 `.cap/capabilities/<kind>/`，或来自 manifest 明示、项目根内、非 symlink、由 lock 覆盖的唯一 Skill source；
   - 每个现有能力与 Skill import 均被引用，或被明确报告为有意未使用。
3. 单独检查 Skill 标准合规：必需 frontmatter、与目录一致的有效小写连字符 `name`、说明触发条件的非空 `description`，以及有效项目内引用。
4. 检查路径卫生：使用规范化 POSIX 项目相对路径，不依赖 symlink、用户级 overlay、provider 原生全局根目录或 secret 文件。
5. 在所有结论中分开证据：
