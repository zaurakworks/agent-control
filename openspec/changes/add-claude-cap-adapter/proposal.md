...
### 决定 1：认证模式默认值 —— ✅ 已确认：订阅 OAuth（2026-08-19）
...
### 决定 2：是否先修 lock 的跨平台缺陷 —— ✅ 已确认：先修（2026-08-19），且已实施

实施本变更时发现一个**与 Claude 无关的既有缺陷**：`.cap/lock.json` 的 `inputs.*.mode` 用平台相关的 `S_IMODE` 记录（Linux `0644` / Windows `0666`），导致 lock 在两平台间翻转。**在 Windows 上，66 个 profile 测试有 65 个因此失败**（在干净 `git HEAD` 上同样失败；CI 是 ubuntu 且不跑 `cap verify`，故抓不到）。
...
### 决定 3：证据天花板的表述 —— ✅ 已确认：照写不弱化（2026-08-19）
