...
本文件是恢复指针，不是状态权威；恢复按 [`README.md`](../README.md)「开始工作」分流；依据 [#22](https://github.com/zaurakworks/agent-control/issues/22)。
...
- 老仓未 archive，「不再写入」是约定不是机制，会漂移。
- 老 #237 未重建；新 Project `Status` 仍为英文默认。

## 防漂移写入规则

目标、授权、范围、决定、验收、等待清单、进度叙事与执行态不进本文件；仅在入口变化、水位推进或冲突增减时更新。
