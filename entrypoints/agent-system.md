...
1. 负责人明确激活公开、自足的 GitHub Issue：读取远端当前正文、状态、标签和明确引用，只在合同授权范围内执行。
2. Issue 带 `迁移索引/待分诊` 标签：默认只分诊和只读核验，不从旧正文、私有评论或开放状态恢复授权。
3. 没有明确 Issue：保持自由对话或当前请求的最小范围；可以提出 proposal，不能自行激活、派发或恢复旧事项。

Session 职责由负责人当前指令、公开自足合同和写入所有权决定。`github-collaboration` 等 Plugin 只有在实际安装、合同明确调用且运行时核验通过时才作为可选工作流；源码存在不等于生效。

Issue 不能覆盖更高层权限边界。合同、运行能力或写入所有权有冲突时，保持相关范围只读并升级。
...
- 在线 Session 自然发现直接对应公开父 Issue 的缺口时，只记录 proposal；只有负责人明确激活后，才按公开、自足的当前合同执行。`github-collaboration:issue-workflow` 仅在实际安装、合同明确调用且运行时核验通过时加载。没有可核验父级或授权时不自动启动，也不扫描队列找活。
...
当负责人明确要求查看、整理或更新经营组合时，读取 `authority/10-operating-ledger.md`。当前没有公共经营 Project；私有旧 Project 只作历史样本。若 `operating-ledger-maintenance` 已实际安装且当前合同明确调用，可用它生成 proposal 或维护公开对象，但不能自动激活下一项工作。
